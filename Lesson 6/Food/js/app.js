...
function postData(form) {
	form.addEventListener('submit', (e) => {
		...
		fetch('server1.php', {
			...
			showResultModal(message.success)
		}).catch(() => {
			showResultModal(message.fail)
		}).finally(() => {
			form.reset()
			messageBlock.remove()
		})
...
function showResultModal (message) {
	...
	previousModal.classList.add('hade')
	...
	setTimeout(() => {
		previousModal.classList.remove('hade')
		closeModal()
		resultModal.remove
	}, 2000)
